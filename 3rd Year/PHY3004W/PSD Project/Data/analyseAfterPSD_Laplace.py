...
longInts = longInts1[Laplace_ChiSq<10]
Laplace_Res = Laplace_Res1[Laplace_ChiSq<10]
Laplace_Poles = Laplace_Poles1[Laplace_ChiSq<10]
...
# region comparing to CCM STNG but with low chisq
...
# plt.hist2d(longInts[np.abs(Laplace_PSDs)<5], Laplace_PSDs[np.abs(Laplace_PSDs)<5], bins=500, norm=colors.LogNorm())
plt.xlabel('$L$ (MeVee)')
plt.ylabel('$S_{PL}$ (a.u.)')
plt.colorbar(label='Counts')
plt.show()
# plt.savefig('Plots/PL_AmBe_hist2d_2.png')
...
# plt.hist(Laplace_ChiSq[Laplace_ChiSq<10], bins=5)
# plt.show()
