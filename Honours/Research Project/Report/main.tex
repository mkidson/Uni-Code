...
\section{Conclusions}
Run 3 at the LHC, with its increased energy and luminosity, brings a number of new challenges that required the ALICE detectors and analysis framework to be upgraded. Among the detector upgrades were two new silicon tracking detectors; the ITS and MFT. In this report we used the new O2 analysis framework to look at histograms of variables relating to particle tracks in either the ITS or MFT. The intention was both to learn the new framework and to determine if the new detectors were performing as expected.

The basics of creating and running an analysis task in O2 were explained, which were then used to analyse tracks of particles detected in each detector. Using the assumption of isotropic emission of particles from IP as a baseline, we interpreted histograms of kinematic variables through the lens of the geometry of the sensitive material of the detectors as well as the geometry of the coordinates commonly used in particle physics. This led to expectations of smooth distributions in most cases 
