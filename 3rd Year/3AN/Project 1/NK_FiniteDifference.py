...
# This is just a order 4 approx to the first derivative. probs not necessary but it's here
# def UPrime(arr):
#     out=[]
#     out.append((-3*arr[0]+4*arr[1]-arr[2])/(2*h))
#     out.append((arr[2]-arr[0])/(2*h))
#     for ind, elem in enumerate(arr[1:-1]):
#         if ind==0 or ind==1:
#             pass
#         else:
#             out.append((arr[ind-2]-8*arr[ind-1]+8*arr[ind+1]-arr[ind+2])/(12*h))
#     out.append((arr[-1]-arr[-3])/(2*h))
